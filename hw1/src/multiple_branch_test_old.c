...
/*
 * return -1 if any files cannot be opened
 * return 0 if two files are different
 * return 1 if same
 */
int compare_two_files(char* path1, char* path2) {
    FILE *fp1, *fp2;
    int ch1, ch2;

    fp1 = fopen(path1, "r");
    if(fp1 == NULL) {
        printf("ERROR:%s cannot be opened", path1);
        return -1;
    }

    fp2 = fopen(path2, "r");
    if(fp2 == NULL) {
        printf("ERROR:%s cannot be opened", path2);
        return -1;
    }
...
Test(multiple_branch_suite, branch16) {
    char *cmd = "cmp <(bin/hw1 -a < test_rsrc/branch16.asm) ";
}
